...
class List
  ...
  def car
    @inner.nil? ? @car : @inner[0]
  end

  def cdr
    if @inner
      @inner.size <= 1 ? EmptyList.instance : List.create(@inner[1..-1], nil)
    else
      if @cdr.is_a?(LyraFn)
  ...
  def self.create(head, tail)
    if head.is_a?(Array) && tail.nil?
      if head.empty?
        EmptyList.instance
      else
        List.send :new, head, nil, nil
      end
    elsif tail.is_a? LazyList
      ...
      raise LyraError.new("Illegal cdr #{tail.inspect}.", :"illegal-argument")
    end
  end
...
def list(*args)
  if args.empty?
    ...
    List.create args[0], EmptyList.instance
  else
